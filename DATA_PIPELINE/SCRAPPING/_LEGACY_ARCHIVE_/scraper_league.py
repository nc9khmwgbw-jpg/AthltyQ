...
try:
    from sofascore_match_scraper import extraire_matchs_joueur, creer_driver, get_players_in_team, get_player_age  # type: ignore
except ImportError:
...
LEAGUES = {
    ...
    "4":  {"id": 23,  "name": "SerieA",            "country": "Italie",     "url": "https://www.sofascore.com/tournament/football/italy/serie-a/23"},
    "5":  {"id": 35,  "name": "Bundesliga",        "country": "Allemagne",  "url": "https://www.sofascore.com/tournament/football/germany/bundesliga/35"},
    "6":  {"id": 7,   "name": "ChampionsLeague",   "country": "Europe",     "url": "https://www.sofascore.com/tournament/football/europe/uefa-champions-league/7"},

    # ── Nouvelles ligues ────────────────────────────────────────────────
    "7":  {"id": 37,  "name": "Eredivisie",        "country": "Pays-Bas",   "url": "https://www.sofascore.com/tournament/football/netherlands/eredivisie/37"},
    "8":  {"id": 38,  "name": "ProLeague",         "country": "Belgique",   "url": "https://www.sofascore.com/tournament/football/belgium/jupiler-pro-league/38"},
    "9":  {"id": 955, "name": "SaudiProLeague",    "country": "Arabie",     "url": "https://www.sofascore.com/tournament/football/saudi-arabia/saudi-professional-league/955"},
    "10": {"id": 18,  "name": "Championship",      "country": "Angleterre", "url": "https://www.sofascore.com/tournament/football/england/championship/18"},
    "11": {"id": 670, "name": "SuperLeagueGreece", "country": "Grece",      "url": "https://www.sofascore.com/tournament/football/greece/super-league/670"},
    "12": {"id": 52,  "name": "SuperLigTurquie",   "country": "Turquie",    "url": "https://www.sofascore.com/tournament/football/turkey/super-lig/52"},
    "13": {"id": 36,  "name": "ScottishPrem",      "country": "Ecosse",     "url": "https://www.sofascore.com/tournament/football/scotland/premiership/36"},
    "14": {"id": 238, "name": "LigaPortugal",      "country": "Portugal",   "url": "https://www.sofascore.com/tournament/football/portugal/liga-portugal/238"},
    "15": {"id": 242,  "name": "MLS",               "country": "USA",        "url": "https://www.sofascore.com/tournament/football/usa/mls/242"},
    "16": {"id": 1322, "name": "UAEProLeague",       "country": "EAU",        "url": "https://www.sofascore.com/tournament/football/united-arab-emirates/uae-pro-league/1322"},
}
    ...
    try:
        ...
        for i, team_url in enumerate(team_links, 1):
            ...
            for j, p in enumerate(players, 1):
                ...
                if file_path.exists():
                    # Toujours patcher l'âge s'il manque, même en cas de mise à jour forcée
                    try:
                        df_existing = pd.read_csv(file_path, encoding='utf-8-sig')
                        if 'Age' not in df_existing.columns or df_existing['Age'].isna().any():
                            age = get_player_age(p_id, driver)
                            if age is not None:
                                df_existing['Age'] = age
                                df_existing.to_csv(file_path, index=False, encoding='utf-8-sig')
                                print(f"      ⏩ 📅 Age patché : {age} ans pour {p_name}")
                    except:
                        pass
                ...
                if is_update:
                    try:
                        df_old_temp = pd.read_csv(file_path, encoding='utf-8-sig')
...
def main():
    ...
    print("  [ALL] Toutes les ligues (Lancement global)")
    ...
    if choix == "ALL":
        ...
        for k in LEAGUES.keys():
            try:
                scrape_league(k, force_update=force_update)
            except Exception as e:
                print(f"❌ Erreur critique sur la ligue {LEAGUES[k]['name']} : {e}")
                continue
    elif choix in LEAGUES:
        rep = input("🔄 Mettre à jour les joueurs existants ? (o/N) : ").strip().lower()
        force_update = (rep == 'o')
